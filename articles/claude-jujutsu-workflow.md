...
正直、`git add` `commit` `push`にやっと慣れたレベル。他のGitコマンド、難しくない？

俺はZedを使って4つのプロジェクトを並行開発してるんだけど、ある日コンフリクトが起きた。Claudeに「`git stash`しましょう」って言われるがまま実行したら、なぜか別プロジェクトでstashが発動してて、気づかないまま作業が巻き戻ってた。Claudeも俺も混乱。何が起きたのかわからない。

**あの絶望感、わかる人にはわかるはず。**

今、俺は**大量のプロジェクトを含むモノレポ**をClaude Code（AIアシスタント）と一緒に開発してる。で、コミットやプッシュするのは**俺じゃなくてClaude**なんだよね。
